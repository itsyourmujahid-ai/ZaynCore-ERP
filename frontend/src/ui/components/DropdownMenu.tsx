// ============================================================================
// Sleek Accessible Dropdown Menu Component (Adaptive Semantic Theming)
// ============================================================================
    ...
    <div className={`relative inline-block text-left shrink-0 ${className}`} ref={dropdownRef}>
      {variant === 'dots' ? (
        ...
        <button
          type="button"
          onClick={() => setIsOpen(!isOpen)}
          className="inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 text-xs font-semibold text-foreground hover:bg-muted rounded-xl transition-colors cursor-pointer"
          aria-expanded={isOpen}
        >
          <span>{label}</span>
        ...
        <button
          type="button"
          onClick={() => setIsOpen(!isOpen)}
          className="inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 text-xs font-semibold bg-card border border-border hover:border-muted-foreground/50 text-foreground rounded-xl shadow-sm transition-all cursor-pointer"
          aria-expanded={isOpen}
        >
          <span>{label}</span>
          <ChevronDown className={`w-3.5 h-3.5 text-muted-foreground transition-transform shrink-0 ${isOpen ? 'rotate-180' : ''}`} />
