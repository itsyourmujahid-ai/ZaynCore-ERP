  ...
  return (
    <div className="space-y-6">
          ...
          >
            <div className="p-4 rounded-xl bg-card/60 border border-border space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                <div className="space-y-2 bg-muted/50 p-3 rounded-lg border border-border/60">
                  <span className="font-semibold text-foreground/90 block">Sub-Ledger Accruals & Outflows:</span>
                  <div className="flex justify-between text-muted-foreground">
                    <span>(+) Total Posted Payroll Batches:</span>
                    <span className="text-foreground font-mono">${parseFloat(reconciliation.details.totalPostedPayrollAccrued).toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between text-muted-foreground">
                    <span>(+) Final Settlements Accrued:</span>
                    <span className="text-foreground font-mono">${parseFloat(reconciliation.details.totalFinalSettlementsAccrued).toFixed(2)}</span>
                  </div>
                  ...
                  <div className="flex justify-between text-foreground font-bold border-t border-border pt-1.5">
                ...
                <div className="space-y-2 bg-muted/50 p-3 rounded-lg border border-border/60">
                  <span className="font-semibold text-foreground/90 block">General Ledger Account #2300:</span>
                  <div className="flex justify-between text-muted-foreground">
                    <span>Control Account Code:</span>
                    <span className="text-foreground font-mono">{reconciliation.glControlAccountCode}</span>
                  </div>
                  <div className="flex justify-between text-muted-foreground">
                    <span>Account Description:</span>
                    <span className="text-foreground">{reconciliation.glControlAccountName}</span>
                  </div>
                  <div className="flex justify-between text-muted-foreground">
                    <span>Account Normal Balance:</span>
                    <span className="text-foreground font-mono">CREDIT</span>
                  </div>
                  <div className="flex justify-between text-foreground font-bold border-t border-border pt-1.5">
        ...
        <Card
          ...
          action={
            <select
              ...
              className="px-3 py-1.5 bg-card border border-border rounded-lg text-xs text-foreground focus:outline-none focus:border-brand-500"
            >
              {periods.map((p) => (
              ...
              <table className="w-full text-left text-xs">
                ...
                <tbody className="divide-y divide-border">
                  {periodEntries.map((pe) => (
                    <tr key={pe.id} className="hover:bg-muted/20 transition-colors">
                      <td className="p-3">
                        <div className="font-semibold text-foreground">{pe.employeeName}</div>
                        <div className="text-[10px] text-muted-foreground font-mono">{pe.employeeCode}</div>
                      </td>
                      <td className="p-3 font-mono text-foreground/90">${parseFloat(pe.basicSalary).toFixed(2)}</td>
                      <td className="p-3 font-mono text-foreground/90">${parseFloat(pe.totalAllowances).toFixed(2)}</td>
                      <td className="p-3 font-mono text-amber-400">${parseFloat(pe.totalOvertime).toFixed(2)}</td>
                      <td className="p-3 font-mono font-semibold text-foreground">${parseFloat(pe.grossSalary).toFixed(2)}</td>
      ...
      {activeReport === 'department_expense' && (
        ...
        >
          <div className="p-4 rounded-xl bg-card/60 border border-border text-xs text-foreground/90 space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-4 rounded-xl bg-card/60 border border-border">
                <span className="text-muted-foreground text-xs block">Engineering & Technology</span>
                <div className="text-xl font-bold text-foreground mt-1">$24,500.00</div>
                <span className="text-[10px] text-muted-foreground">4 Employees (Basic: $18k, Allow: $4.5k, OT: $2k)</span>
              </div>
              <div className="p-4 rounded-xl bg-card/60 border border-border">
                <span className="text-muted-foreground text-xs block">Sales & Marketing</span>
                <div className="text-xl font-bold text-foreground mt-1">$16,200.00</div>
                <span className="text-[10px] text-muted-foreground">3 Employees (Basic: $12k, Allow: $3.2k, OT: $1k)</span>
              </div>
              <div className="p-4 rounded-xl bg-card/60 border border-border">
                <span className="text-muted-foreground text-xs block">Operations & Admin</span>
                <div className="text-xl font-bold text-foreground mt-1">$11,800.00</div>
                <span className="text-[10px] text-muted-foreground">2 Employees (Basic: $9k, Allow: $2.3k, OT: $500)</span>
              </div>
            </div>
          </div>
        </Card>
