...
export const FutureModulePlaceholder: React.FC<{ moduleKey: string; onNavigate: (viewId: string) => void }> = ({
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      {/* Top Banner */}
      <div className="p-8 rounded-2xl bg-gradient-to-r from-slate-900 via-slate-900 to-brand-950/30 border border-slate-800 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-100 mt-1">{modDef.name}</h1>
            <p className="text-xs text-slate-400 mt-1 max-w-xl">{modDef.description}</p>
          </div>
        </div>

        <div className="flex flex-col items-end gap-2 shrink-0">
          <div className="text-right">
            <span className="text-[10px] text-slate-500 uppercase font-bold block">Company Entitlement</span>
            <span className="text-xs font-semibold text-slate-300">
              {isEnabled ? (
                <span className="text-emerald-400">Enabled for {tenant.companyTier.toUpperCase()}</span>
              ) : (
                <span className="text-slate-500">Disabled (Requires {modDef.minTier.toUpperCase()})</span>
              )}
            </span>
          </div>
          <Button
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {modDef.subFeatures.map((f) => (
            <div key={f.key} className="p-3.5 rounded-lg bg-slate-950/60 border border-slate-800 flex flex-col justify-between">
              <div>
              ...
              <div className="mt-2 text-[10px] font-mono text-slate-500">Key: {f.key}</div>
            </div>
          ))}
        </div>
      </Card>
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
          {modDef.permissions.map((p) => (
            <div key={p.code} className="p-3 rounded-lg bg-slate-950/50 border border-slate-800 text-xs">
