    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <span className="text-xs font-bold uppercase tracking-wider text-purple-600 dark:text-purple-400">VVIP Platform Super Admin</span>
            <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-purple-500/10 text-purple-700 dark:text-purple-400 border border-purple-500/20">
              CROSS-TENANT CONTROL
            </span>
          </div>
          <h1 className="text-2xl font-bold text-foreground mt-1">Tenant & Platform Management</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-4 rounded-xl bg-card border border-border flex items-center justify-between shadow-sm">
          <div>
            <span className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider block">Total Organizations</span>
            <span className="text-2xl font-bold text-foreground mt-1 block font-mono">{companies.length}</span>
            <span className="text-[10px] text-muted-foreground mt-0.5 block">{companies.length === 0 ? 'Clean Fresh Baseline' : 'Configured Tenants'}</span>
          </div>
        ...
        <div className="p-4 rounded-xl bg-card border border-border flex items-center justify-between shadow-sm">
          <div>
            <span className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider block">Active Tenants</span>
            <span className="text-2xl font-bold text-emerald-600 dark:text-emerald-400 mt-1 block font-mono">{activeCompaniesCount}</span>
            <span className="text-[10px] text-muted-foreground mt-0.5 block">Operational Status</span>
          </div>
        ...
        <div className="p-4 rounded-xl bg-card border border-border flex items-center justify-between shadow-sm">
          <div>
            <span className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider block">Inactive / Suspended</span>
            <span className="text-2xl font-bold text-amber-600 dark:text-amber-400 mt-1 block font-mono">{inactiveCompaniesCount}</span>
            <span className="text-[10px] text-muted-foreground mt-0.5 block">Deactivated Access</span>
          </div>
        ...
        <div className="p-4 rounded-xl bg-card border border-border flex items-center justify-between shadow-sm">
          <div>
            <span className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider block">System Health</span>
            <span className="text-sm font-bold text-purple-600 dark:text-purple-300 mt-1 block">100% Operational</span>
            <span className="text-[10px] text-muted-foreground mt-0.5 block">Strict Tenant Isolation Active</span>
          </div>
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {drafts.map((draft) => (
              <div
                key={draft.id}
                className="p-4 rounded-xl bg-card border border-amber-500/30 hover:border-amber-500/50 transition-all flex flex-col justify-between space-y-3 shadow-sm"
              >
                <div>
                  <div className="flex items-center justify-between">
                    ...
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-amber-500/10 text-amber-700 dark:text-amber-400 border border-amber-500/20 shrink-0">
                      Step {draft.currentStep} of 12
                    </span>
                  </div>

                  <div className="mt-2 text-xs text-muted-foreground flex items-center gap-2">
                    <span>Type: <strong className="text-foreground capitalize">{draft.payload?.businessTypes?.join(', ') || 'Unassigned'}</strong></span>
                  </div>
      ...
      >
        {companies.length === 0 ? (
          <div className="p-8 text-center space-y-4 max-w-md mx-auto">
            ...
            <div>
              <h3 className="text-base font-bold text-foreground">Zero Customer Companies (Fresh State)</h3>
                    ...
                    <tr key={comp.id} className="hover:bg-muted/40 transition-colors">
                      <td className="px-5 py-4 max-w-xs">
                        <div className="flex items-center gap-3 min-w-0">
                          <div className="w-8 h-8 rounded-lg bg-muted border border-border flex items-center justify-center text-foreground font-bold shrink-0">
                            {comp.code.slice(0, 2)}
                          </div>
                          <div className="min-w-0 flex-1">
                            <div className="font-semibold text-foreground flex items-center gap-2 truncate" title={comp.name}>
                              <span className="truncate">{comp.name}</span>
                              <span className="font-mono text-[10px] text-muted-foreground shrink-0">[{comp.code}]</span>
                            </div>
                            <div className="text-[11px] text-muted-foreground truncate" title={`${comp.legalName} • ${comp.industry}`}>
                      ...
                      <td className="px-5 py-4">
                        <span className="px-2 py-0.5 rounded bg-muted border border-border text-foreground text-xs font-medium">
