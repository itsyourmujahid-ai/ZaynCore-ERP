              ...
              {filteredOrders.map((o) => {
                ...
                return (
                  <tr key={o.id} className="hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="px-5 py-3.5 font-mono text-slate-300">{o.orderDate}</td>
                    <td className="px-5 py-3.5 font-mono text-slate-400">{o.deliveryDate || 'N/A'}</td>
                    <td className="px-5 py-3.5 text-right font-mono text-emerald-400">${parseFloat(o.invoicedAmount).toFixed(2)}</td>
                    <td className="px-5 py-3.5 text-right font-mono font-bold text-slate-100">${parseFloat(o.total).toFixed(2)} {o.currency}</td>
        ...
        >
          <div className="space-y-4 text-xs">
            <div className="p-3.5 rounded-lg bg-slate-950/70 border border-slate-800 grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div>
                <span className="text-slate-500 text-[10px] uppercase font-bold block">Order Date</span>
                <span className="text-slate-200 font-mono mt-0.5">{selectedOrder.orderDate}</span>
              </div>
              <div>
                <span className="text-slate-500 text-[10px] uppercase font-bold block">Delivery Target</span>
                <span className="text-slate-200 font-mono mt-0.5">{selectedOrder.deliveryDate || 'N/A'}</span>
              </div>
              <div>
                <span className="text-slate-500 text-[10px] uppercase font-bold block">Invoiced So Far</span>
                <span className="text-emerald-400 font-mono font-bold mt-0.5">${parseFloat(selectedOrder.invoicedAmount).toFixed(2)}</span>
              </div>
              <div>
                <span className="text-slate-500 text-[10px] uppercase font-bold block">Total Contract</span>
                <span className="text-slate-100 font-mono font-bold mt-0.5">${parseFloat(selectedOrder.total).toFixed(2)} {selectedOrder.currency}</span>
              </div>
            </div>
