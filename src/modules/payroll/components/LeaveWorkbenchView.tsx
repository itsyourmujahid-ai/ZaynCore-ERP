  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-base font-bold text-foreground">Leave Management & Accruals</h2>
          <p className="text-xs text-muted-foreground">Review time-off requests, manage vacation balances, and approve leaves.</p>
        </div>
      ...
      <Card
        ...
        action={
          <select
            ...
            className="px-2.5 py-1 bg-card border border-border rounded-lg text-xs text-foreground/90 focus:outline-none focus:border-brand-500"
          >
                ...
                {filteredRequests.map((req) => {
                  ...
                  return (
                    <tr key={req.id} className="hover:bg-muted/20 transition-colors">
                      <td className="p-3 font-semibold text-foreground">{req.requestNumber}</td>
                      <td className="p-3">
                        <div className="font-semibold text-foreground">{emp?.fullName || req.employeeId}</div>
                        <div className="text-[10px] text-muted-foreground">{emp?.employeeCode}</div>
                      </td>
                      <td className="p-3 font-medium text-foreground">{lt?.name || 'Leave'}</td>
                      <td className="p-3 text-muted-foreground">
                        {req.startDate} → {req.endDate}
                      </td>
                      <td className="p-3 font-semibold text-foreground">{req.numberOfDays} days</td>
                      ...
                      <td className="p-3 text-right">
                        {req.status === 'submitted' ? (
                          ...
                          <span className="text-muted-foreground text-[10px]">Processed</span>
                        )}
                      </td>
                    </tr>
                  );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-card/80 backdrop-blur-sm animate-fadeIn">
          <div className="bg-card border border-border rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden">
            <div className="p-5 border-b border-border flex items-center justify-between">
              <h2 className="text-base font-bold text-foreground">Apply for Leave of Absence</h2>
            ...
            <form onSubmit={handleApplyLeave} className="p-6 space-y-4 text-xs">
              ...
              <div>
                <label className="block text-muted-foreground mb-1">Number of Days *</label>
                <input
                  type="number"
                  step="0.5"
                  required
              ...
              <div>
                <label className="block text-muted-foreground mb-1">Reason for Leave *</label>
                <textarea
                  required
                  rows={3}
                  value={formReason}
