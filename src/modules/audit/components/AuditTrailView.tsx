    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold text-foreground mt-1">Immutable Audit Trail</h1>
            ...
            <tbody className="divide-y divide-border text-foreground">
              {filteredLogs.map((log) => (
                <tr key={log.id} className="hover:bg-muted/40 transition-colors">
                  ...
                  <td className="px-5 py-3.5">
                    <span className="font-mono text-foreground/90 bg-muted px-1.5 py-0.5 rounded text-[11px]">
                      {log.entityType}#{log.entityId.slice(0, 8)}
                    </span>
                  </td>
                  <td className="px-5 py-3.5 text-foreground/90 max-w-sm truncate">
                    {log.details}
                  </td>
      ...
      {selectedLog && (
        ...
        >
          <div className="space-y-4 text-xs">
            <div className="grid grid-cols-2 gap-3 p-3 bg-card/70 rounded-lg border border-border">
              <div>
                <span className="text-muted-foreground block text-[10px] uppercase font-bold">Actor Email</span>
                <span className="font-semibold text-foreground mt-0.5 block">{selectedLog.userEmail}</span>
              </div>
              <div>
                <span className="text-muted-foreground block text-[10px] uppercase font-bold">Target Entity</span>
                <span className="font-mono text-foreground mt-0.5 block">{selectedLog.entityType} ({selectedLog.entityId})</span>
              </div>
            </div>

            <div className="p-3 bg-card/70 rounded-lg border border-border">
              <span className="text-muted-foreground block text-[10px] uppercase font-bold">Audit Narrative</span>
              <p className="text-foreground mt-1 font-medium">{selectedLog.details}</p>
            </div>

            {/* State Diffs (Previous vs New) */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <span className="text-muted-foreground font-bold block mb-1">Previous State</span>
                <pre className="p-3 rounded-lg bg-card border border-border font-mono text-[11px] text-muted-foreground overflow-x-auto max-h-48">
              ...
              <div>
                ...
                <pre className="p-3 rounded-lg bg-card border border-border font-mono text-[11px] text-emerald-300 overflow-x-auto max-h-48">
