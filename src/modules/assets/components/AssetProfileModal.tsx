        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400 mt-0.5">
                Category: <span className="text-slate-200 font-medium">{category?.name || 'Unassigned'}</span> • Tag: <span className="font-mono text-blue-400">{asset.tagNumber || 'N/A'}</span>
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-6">
            <div className="text-right">
              <div className="text-xs text-slate-400 font-medium">Original Cost</div>
              <div className="text-lg font-bold text-slate-100">${cost.toLocaleString('en-US', { minimumFractionDigits: 2 })}</div>
            </div>
            <div className="text-right">
              <div className="text-xs text-slate-400 font-medium">Accum. Depreciation</div>
              <div className="text-lg font-bold text-amber-400">-${accumDep.toLocaleString('en-US', { minimumFractionDigits: 2 })}</div>
            </div>
            <div className="text-right pl-4 border-l border-slate-800">
              <div className="text-xs text-slate-400 font-medium">Net Book Value</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <Card className="p-5 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div>
                    <span className="text-xs text-slate-400 block">Asset Code</span>
                    <span className="font-mono font-medium text-slate-200">{asset.assetCode}</span>
                  </div>
                  <div>
                    <span className="text-xs text-slate-400 block">Asset Type</span>
                    <span className="capitalize text-slate-200">{asset.assetType}</span>
                  </div>
                  <div>
                    <span className="text-xs text-slate-400 block">Serial Number</span>
                    <span className="font-mono text-slate-200">{asset.serialNumber || 'N/A'}</span>
                  </div>
                  <div>
                    <span className="text-xs text-slate-400 block">Barcode / RFID Tag</span>
                    <span className="font-mono text-blue-400">{asset.tagNumber || 'N/A'}</span>
                  </div>
                  <div>
                    <span className="text-xs text-slate-400 block">Purchase Date</span>
                    <span className="text-slate-200">{asset.purchaseDate || 'N/A'}</span>
                  </div>
                  <div>
                    <span className="text-xs text-slate-400 block">Capitalization Date</span>
                    <span className="text-slate-200">{asset.capitalizationDate || 'Uncapitalized (Draft)'}</span>
                  </div>
                </div>
              </Card>
              ...
              <Card className="p-5 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div>
                    <span className="text-xs text-slate-400 block">Responsible Custodian</span>
                    <span className="font-medium text-slate-200">{asset.custodianName || 'Unassigned'}</span>
                  </div>
                  <div>
                    <span className="text-xs text-slate-400 block">Physical Location</span>
                    <span className="text-slate-200">{asset.location || 'Central Facility'}</span>
                  </div>
                  <div>
                    <span className="text-xs text-slate-400 block">Cost Center</span>
                    <span className="text-slate-200 font-mono">{asset.costCenterId || 'Corporate General'}</span>
                  </div>
                  <div>
                    <span className="text-xs text-slate-400 block">Assigned Project</span>
                    <span className="text-slate-200">{asset.projectId || 'None'}</span>
                  </div>
                </div>
              </Card>
              ...
              <Card className="p-5 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div>
                    ...
                    <span className="capitalize text-slate-200">{asset.depreciationMethod.replace('_', ' ')}</span>
                  </div>
                  <div>
                    <span className="text-xs text-slate-400 block">Useful Life</span>
                    <span className="text-slate-200 font-semibold">{asset.usefulLifeMonths} Months ({(asset.usefulLifeMonths / 12).toFixed(1)} Yrs)</span>
                  </div>
                  <div>
                    <span className="text-xs text-slate-400 block">Salvage / Residual Value</span>
                    <span className="text-slate-200 font-mono">${parseFloat(asset.residualValue || '0').toFixed(2)}</span>
                  </div>
                  <div>
                    <span className="text-xs text-slate-400 block">Depreciation Progress</span>
              ...
              <Card className="p-5 space-y-4">
                ...
                <div className="space-y-2 text-xs">
                  <div className="flex justify-between py-1 border-b border-slate-800">
                    ...
                    <span className="font-mono text-slate-200">#1510 Property, Plant & Equipment</span>
                  </div>
                  <div className="flex justify-between py-1 border-b border-slate-800">
                    <span className="text-slate-400">Accumulated Depreciation:</span>
                    <span className="font-mono text-slate-200">#1520 Accumulated Depreciation</span>
                  </div>
                  <div className="flex justify-between py-1 border-b border-slate-800">
                    <span className="text-slate-400">Depreciation Expense:</span>
                    <span className="font-mono text-slate-200">#6020 Depreciation Expense</span>
                  </div>
                  <div className="flex justify-between py-1">
                    <span className="text-slate-400">Gain / Loss on Disposal:</span>
                    <span className="font-mono text-slate-200">#4085 / #6085 Gain/Loss on Sale</span>
                  </div>
                </div>
              </Card>
            </div>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <div className="text-sm text-slate-400">
                Periodic monthly straight-line depreciation schedule. Total periods: <span className="font-bold text-slate-200">{scheduleLines.length}</span>
              </div>
            ...
            <div className="border border-slate-800 rounded-xl overflow-hidden max-h-96 overflow-y-auto">
              <table className="w-full text-left text-sm">
                <thead className="bg-slate-900/90 text-xs font-semibold text-slate-400 uppercase tracking-wider sticky top-0">
                  <tr>
                ...
                <tbody className="divide-y divide-slate-800 font-mono text-xs">
                  {scheduleLines.map((line) => (
                    <tr key={line.id} className="hover:bg-slate-800/40">
                      <td className="p-3 font-sans font-medium text-slate-300">{line.periodName}</td>
                      <td className="p-3 text-right text-slate-400">${parseFloat(line.openingNBV).toFixed(2)}</td>
                      <td className="p-3 text-right font-bold text-amber-400">${parseFloat(line.depreciationAmount).toFixed(2)}</td>
                      <td className="p-3 text-right text-slate-300">${parseFloat(line.accumulatedDepreciation).toFixed(2)}</td>
                      ...
                      <td className="p-3 text-center">
                        {line.isPosted ? (
                          ...
                          <span className="text-[11px] font-sans text-slate-400 bg-slate-800 px-2 py-0.5 rounded-full">
                            Scheduled
                          </span>
                        )}
                      </td>
                  ...
                  {asset.components && asset.components.length > 0 ? (
                    asset.components.map((c) => (
                      <tr key={c.id}>
                        ...
                        <td className="p-3 font-mono text-slate-400">{c.serialNumber || '—'}</td>
                        <td className="p-3 text-right font-mono text-slate-200">${parseFloat(c.cost).toFixed(2)}</td>
                        <td className="p-3 text-right">{c.usefulLifeMonths} Mos</td>
                        <td className="p-3 text-slate-400">{c.notes || '—'}</td>
                      </tr>
                    ))
                  ) : (
                    <tr>
                ...
                <tbody className="divide-y divide-slate-800 text-xs">
                  {transfers.map((t) => (
                    <tr key={t.id}>
                      ...
                      <td className="p-3 text-slate-400">{t.transferDate}</td>
                      <td className="p-3 text-slate-300">{t.fromLocation || '—'} ({t.fromCustodian || '—'})</td>
                      <td className="p-3 font-medium text-emerald-400">{t.toLocation || '—'} ({t.toCustodian || '—'})</td>
                      <td className="p-3 text-slate-400">{t.reason || 'Relocation'}</td>
                    </tr>
                  ))}
                  {transfers.length === 0 && (
                    <tr>
                ...
                <tbody className="divide-y divide-slate-800 font-mono text-xs">
                  {journals.map((j) => (
                    <tr key={j.id} className="hover:bg-slate-800/40">
                      ...
                      <td className="p-3 text-slate-400 font-sans">{j.postingDate}</td>
                      <td className="p-3 font-sans">
                        <span className="bg-slate-800 text-slate-200 px-2 py-0.5 rounded text-[11px]">
                          {j.postingEvent}
                        </span>
                      </td>
                      <td className="p-3 font-sans text-slate-300">{j.memo}</td>
                      <td className="p-3 text-right text-emerald-400 font-bold">${parseFloat(j.totalDebit).toFixed(2)}</td>
                      <td className="p-3 text-right text-slate-300">${parseFloat(j.totalCredit).toFixed(2)}</td>
                    </tr>
                  ))}
                  {journals.length === 0 && (
                    <tr>
