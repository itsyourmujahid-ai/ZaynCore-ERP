...
export const SettingsWorkspace: React.FC<{ initialTab?: string; onNavigate?: (viewId: string) => void }> = ({ 
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <span className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Enterprise Administration</span>
            <span className="text-muted-foreground">•</span>
            <StatusBadge status={tenant.companyTier} />
          </div>
          <h1 className="text-2xl font-bold text-foreground mt-1">Settings & Governance</h1>
      ...
      {/* Categorized Tab Bar */}
      <div className="flex items-center border-b border-border pb-px gap-1 overflow-x-auto">
        {categories.map((cat) => {
      ...
      {/* Categorized Content */}
      <ErrorBoundary fallbackTitle="Settings Section Unavailable" fallbackSubtitle="An issue occurred loading this settings domain.">
            ...
            <Card title="Ledger Currency & Multi-Currency Setup" subtitle="Base currency and FX exchange rate configuration">
              <div className="p-4 rounded-xl bg-muted border border-border flex items-center justify-between text-xs">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="font-semibold text-foreground block">Primary Base Currency: {tenant.baseCurrency}</span>
                    <span className="text-muted-foreground text-[11px]">All General Ledger debits and credits are posted and balanced in {tenant.baseCurrency}.</span>
                  </div>
                </div>
                <span className="px-2.5 py-1 rounded text-[10px] font-bold bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-500/20">
                  ACTIVE
                </span>
              </div>
            </Card>
