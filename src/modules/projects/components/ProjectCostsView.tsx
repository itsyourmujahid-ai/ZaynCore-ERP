        ...
        <div className="flex items-center gap-2 w-full sm:w-auto flex-1 max-w-md">
          <div className="relative w-full">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-1.5 bg-slate-950 border border-slate-700 rounded text-xs text-slate-100 placeholder-slate-500 focus:outline-none"
            />
          </div>
        </div>
          ...
          <table className="w-full text-left text-xs text-slate-300">
            ...
            <tbody className="divide-y divide-slate-800">
              {filteredCosts.map((c) => {
                ...
                return (
                  <tr key={c.id} className="hover:bg-slate-800/40 font-mono">
                    <td className="p-3 text-slate-400">{c.transactionDate}</td>
                    <td className="p-3 font-sans font-bold text-cyan-400">{p?.code || c.projectId}</td>
                    <td className="p-3 font-sans capitalize font-semibold text-slate-200">{c.costCategory}</td>
                    <td className="p-3 text-[11px] text-purple-400 uppercase font-mono">
                      {c.sourceModule}:{c.sourceType}
                    </td>
                    <td className="p-3 text-slate-400">{c.documentNumber}</td>
                    <td className="p-3 font-sans text-slate-300">{c.description}</td>
                    <td className="p-3 text-right font-bold text-slate-100">${parseFloat(c.amount).toLocaleString()}</td>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl max-w-md w-full p-6 space-y-4 shadow-2xl">
            ...
            <form onSubmit={handleRecordCost} className="space-y-3 text-xs">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-slate-400 mb-1">Incurred Amount ($) *</label>
                  <input
                    type="number"
                    step="0.01"
                    required
              ...
              <div className="flex items-center gap-2 pt-1">
                ...
                />
                <label htmlFor="cbBillable" className="text-slate-300">Billable to Client</label>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-xl max-w-md w-full p-6 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              ...
              <button onClick={() => setTimeModalOpen(false)} className="text-slate-400 hover:text-slate-200">✕</button>
            </div>
            ...
            <form onSubmit={handleAllocateLabor} className="space-y-3 text-xs">
              ...
              <div>
                <label className="block text-slate-400 mb-1">Employee / Resource Name *</label>
                <input
                  type="text"
                  required
                  value={employeeName}
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-slate-400 mb-1">Hourly Billing/Cost Rate ($) *</label>
                  <input
                    type="number"
                    step="0.01"
                    required
              ...
              <div className="p-3 bg-slate-950/80 rounded border border-slate-800 text-[11px] text-slate-400">
                ...
                <p className="mt-1 text-slate-500">Automatically executes GL double entry: Dr Project Labor (#5010), Cr Accrued Salaries (#2300).</p>
              </div>
